var ImageDialog = {
  ...
  _populateDirTree : function( dirList )
    ...
    _populateCurrDir : function( dircontents )
    {
        ...
        var httpBaseImageDir = "";

        if( dircontents.getElementsByTagName('basedir')[0].textContent )
        {
            httpBaseImageDir = dircontents.getElementsByTagName('basedir')[0].textContent;  // FF & Mozilla
        }
        else
        {
            httpBaseImageDir = dircontents.getElementsByTagName('basedir')[0].text; // IE, Opera, Safari
        }
    ...
    selectDir : function (currPath)
    {
        ...
        for( var treeIdx = 0; treeIdx < listOfDirsToExpand.length; ++treeIdx )
        {
            ...
            {
                if( treeviewNodes[idx].getAttribute('path') == currNode && treeviewNodes[idx].className == 'folder')
                {
